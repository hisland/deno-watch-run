...
import { dirname, basename, relative } from 'https://deno.land/std@0.151.0/path/mod.ts'
...
const handle = debounce((event: unknown) => {
  const {
    kind,
    paths: [path],
...
function debounce(func: (...args: unknown[]) => unknown, wait: number, fireAtHead = false) {
